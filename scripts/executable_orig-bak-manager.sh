...
fzf_transform="--bind '?:transform:[[ ! \$FZF_PROMPT =~ Sessions ]] && \
  echo \"change-prompt(Sessions> )+reload(get_tmux_sessions)\" || \
  echo \"change-prompt(Zoxide> )+reload(get_zoxide_dirs)\"'"
  # echo \"change-prompt(Tmuxinator> )+reload(get_tmuxinator_projects)\"'"
# echo \"change-prompt(Zoxide> )+reload(zoxide query -l | head -n 10 | xargs -I {x} $HOME/scripts/truncate_path.sh {x})\"'"
# echo \"change-prompt(Tmuxinator Projects> )+reload(tmuxinator list | tail -n +2 | tr \\\" \\\" \\\"\\n\\\" | awk \\\"NF\\\")\"'"

layout=$([[ -n ${TMUX} ]] && echo "--tmux center,50%,50% --layout=reverse" || echo "--height 70% --layout=reverse --margin 15%,15%")
fzf_cmd="fzf --style full"
[ -n "$info" ] && fzf_cmd+=" $info"
[ -n "$border_label" ] && fzf_cmd+=" $border_label"
...
# fzf_cmd="fzf --tmux $fzf_transform"
